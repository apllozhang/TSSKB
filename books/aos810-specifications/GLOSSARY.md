# GLOSSARY — OmniSwitch AOS Release 8 Specifications Guide 核心术语

从 verified 术语库精选 60 条，按主题分组。规格/协议/命令保留英文，页码为原书页码。
...
- **Snapshot（命令捕获）**：把配置捕获成文本文件，含错误报告（<<<PAGE 18>>>）
- **SNMPv3 USM/VACM**：基于用户的安全模型/视图访问控制（RFC 2574/2575），认证 SHA·MD5、加密 DES·AES（<<<PAGE 20>>>）
- **Web Services（Python API）**：HTTP/HTTPS+XML/JSON 北向接口，4 会话，附 consumer.py 示例库（<<<PAGE 21>>>）
- **AMS（AOS Micro Services）**：AOS 微服务框架，全平台支持（<<<PAGE 21>>>）
- **OpenFlow**：仅 6860 支持，Normal/Hybrid 模式、版本 1.0/1.3.1（<<<PAGE 22>>>）
- **RCD（Remote Chassis Detection）**：自动远程配置/零触摸；DHCP 客户端跑在 VLAN 1/tagged VLAN 127/LLDP 管理 VLAN（<<<PAGE 23, 25>>>）
- **Automatic Fabric**：自动 fabric 发现配置，自动配 OSPF/IS-IS（<<<PAGE 26>>>）
- **Automatic LACP**：自动 LACP（tagged VLAN 127、untagged VLAN 1 上传递）（<<<PAGE 25>>>）

## VC 与链路层

- **VC（Virtual Chassis）**：多台交换机组单一逻辑机箱；最大值语义面向整 VC（<<<PAGE 12>>>）
- **VFL（Virtual Fabric Link）**：VC 内部互联链路；peer 数/成员口数因平台而异（<<<PAGE 23>>>）
...
- **集中式 MAC 学习**：规格表 MAC 容量的前提模式（<<<PAGE 30>>>）
...
- **HA VLAN**：服务器集群高可用 VLAN（16-32/VC）（<<<PAGE 32>>>）

## SPB / VXLAN / EVPN

- **SPBM（MAC-in-MAC）**：最短路径桥接的 MAC-in-MAC 模式（802.1aq/802.1ah）（<<<PAGE 33>>>）
...
- **External Loopback Routing**：SPB L3 外环回路由形态，与 Inline 互补（<<<PAGE 34>>>）
- **RFP（Remote Fault Propagation）**：SPB 远端故障传播域（最多 8，与 OAM 域共享）（<<<PAGE 34>>>）
- **SAP（Service Access Point）**：业务接入点；VLAN Stacking 与 SPB/VXLAN 共用概念（<<<PAGE 34, 71>>>）
...
- **LSDB（Link State Database）**：链路状态数据库（OSPF/IS-IS 规格项）（<<<PAGE 79, 81>>>）
- **Route Leaking（IS-IS）**：IS-IS 两级前缀分发（RFC 2966）（<<<PAGE 81>>>）
- **Anycast RP**：任播 RP，PIM-SM 中多 RP 同地址（最多 8）（<<<PAGE 86>>>）
- **MBR（Multicast Border Router）**：PIM-SM 组播边界路由，与 DVMRP 互操作（<<<PAGE 86>>>）
...
- **Fabric profile（6570M/6575）**：面向 SPB fabric 的档位（隧道/UNP 增、QoS/PVLAN 减）（<<<PAGE 90, 92>>>）
